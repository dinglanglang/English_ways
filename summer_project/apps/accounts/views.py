    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if captcha.lower() == session_captcha_code.lower():
                ...
                # user = auth.authenticate(username=username, password=password)
                if flag and user is not None and user.is_active:
                    auth.login(request, user)
...
class PasswordForget(View):
    ...
    def post(self, request):
        ...
        if email and User.objects.filter(email=email):
            ...
            url = "{}://{}/accounts/password/reset/{}?email={}".format(request.scheme,request.META['HTTP_HOST'],verify_code,email)
            ...
            return render(request, "password_forget.html", {"msg":"邮件发送成功，请登录邮箱查看！"})
        else:
            msg = "输入的邮箱不存在！"
